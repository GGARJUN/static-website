...
const Blogs = ({ posts = [] }) => {
    ...
    const categories = useMemo(() => {
        ...
        return ["Category", ...new Set(allCategories)];
    }, [posts]);
    ...
    const filteredPosts = useMemo(() => {
        return posts.filter((post) => {
            ...
            const matchesCategory =
                selectedCategory === "Category" || post.categories.some((cat) => cat.title === selectedCategory);
                    ...
                    {paginatedPosts.length === 0 ? (
                        ...
                        <div className="grid md:grid-cols-3 gap-8 mt-20">
                            {paginatedPosts.map((post) => (
                                <Link key={post._id} href={`/blog/${post.slug.current}`} className="block group border rounded-2xl shadow-xl">
